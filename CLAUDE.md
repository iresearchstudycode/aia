...
| `config.js` | `MODEL_NAME`, `VISION_MODEL_NAME`, `OLLAMA_API_URL`, `OLLAMA_TAGS_URL`, `OLLAMA_MODEL_KEY`, `PIPER_SPEAK_URL`, `VOICEBOX_SPEAK_URL`, `DOC_EXTRACT_URL`, `PERSONA_PREFS_KEY`, `EDITOR_MODE_KEY`, `NAV_RAIL_KEY`, the system prompt objects (11 dropdown personas + the non-dropdown `englishEditorExplained` variant), and global mutable state (`conversationHistory[]`, `currentSystemPrompt`, `currentModel` (defaults to `MODEL_NAME`; the actual model sent for text/thinking turns, set by the toolbar model selector), `pendingImageDataUrl`, `pendingImageBase64`, `currentThinkingMode`, `currentTTSEngine`, `currentEditorMode`, `currentNavRailEnabled` (also mirrored to `window.currentNavRailEnabled` so `nav-rail.js` reads it without a load-order dependency), `pendingDocumentText`, `pendingDocumentName`, `pendingDocumentTruncated`) |
...
- `MODEL_NAME` — the *default* text + thinking model (default: `gemma4:e4b`). Since 1.21.0 this is only the fallback: the actual model sent is `currentModel` (below), chosen in the toolbar model selector.
- `VISION_MODEL_NAME` — vision-capable model for image requests (default: `gemma3:4b`); `gemma4:e4b` has no vision encoder in its GGUF so a separate model is required. **Not selectable** — image turns always route here regardless of the model selector.
- `OLLAMA_API_URL` — proxied chat endpoint (default: `https://localhost/ollama/api/chat`)
- `OLLAMA_TAGS_URL` — proxied model-list endpoint (default: `https://localhost/ollama/api/tags`); GET-only, read once on load to populate the model selector
- `OLLAMA_MODEL_KEY` — `localStorage` key (`'ollamaModel'`) holding the user's selected model
- `currentModel` — runtime global, defaults to `MODEL_NAME`; the model actually sent on every text/thinking turn. Set by the `#modelSelect` toolbar selector (`main.js`), persisted to `localStorage[OLLAMA_MODEL_KEY]`, and restored on load **only if that model is still installed** (else `MODEL_NAME` if installed, else the first installed model). Changeable at any time — no conversation reset, no lock, and not part of `personaPrefs` (it's a global preference, not per-persona). **Known limitation:** `options.num_ctx` is still sent as `OLLAMA_NUM_CTX` (16384) on every request, so picking a model whose context window is smaller than that may make Ollama clamp/truncate context; and the vision model is not selectable in this version (image turns still route to `VISION_MODEL_NAME`).
...
**Toggle:** `#navRailToggle` in `.toolbar-left` (after `#thinkingDepthSelect`) turns the whole rail on/off. Default **on**, persisted to `localStorage['navRailEnabled']` (`NAV_RAIL_KEY`; runtime flag `currentNavRailEnabled` + a `window.currentNavRailEnabled` mirror in `config.js`, wired in `main.js` mirroring the `autoTTS` restore/toggle pattern). This toolbar placement is **explicitly temporary** — the intent is to move it, with other user preferences, onto a dedicated Settings page rather than keep growing the input toolbar.

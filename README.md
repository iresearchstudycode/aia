...
- **Consultant Analysis Templates**: with the **Professional Consultant** persona active, a "Templates" button in the composer offers **SWOT analysis**, **Pros & cons**, and a **Decision matrix**. Picking one scaffolds the prompt and attaches a format instruction; the reply renders as a colour-coded 2×2 SWOT quadrant grid, a two-column pros/cons layout, or — for the matrix — a scored table with a **weight slider (0–5) per criterion** that recomputes the weighted totals and highlights the winning option live (the weights are saved). Every artifact has a **Structured / Text** view switch, and the artifact type / view / matrix weights persist through reload and JSON save/open. Default view is set per-persona in Settings → Personas (`default_analysis_view`)

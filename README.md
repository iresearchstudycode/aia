...
- **Consultant Analysis Templates**: with the **Professional Consultant** persona active, a "Templates" button in the composer offers **SWOT analysis** and **Pros & cons**. Picking one scaffolds the prompt and attaches a format instruction; the reply renders as a colour-coded 2×2 SWOT quadrant grid (or a two-column pros/cons layout) with a **Structured / Text** view switch, and the artifact type + chosen view persist through reload and JSON save/open. Default view is set per-persona in Settings → Personas (`default_analysis_view`)

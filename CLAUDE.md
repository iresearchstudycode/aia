...
| `history.js` | **Conversation history lightbox** (new; loaded after `settings.js`, before `main.js`). Talks to `conversations-service` at `CONVERSATIONS_API_URL` (`config.js`). Globals: `initHistory()` (build the hidden lightbox into `#historyRoot` — idempotent), `openHistory()` / `closeHistory()`, `hcCurrentId()`, `hcNewConversationId()` (mint + set a fresh id), `hcArchiveCurrent()` (async, never throws — `PUT`s the current record before `clearChat()` wipes), `hcTouchCurrent()` (async, coalesced, never throws — the per-turn auto-save, called from `api.js`'s three turn-complete points next to `_refreshTurnControls()`), `hcLoadConversation(id)` (async — `GET` + `renderConversationHistory()` + set current id), `hcBeaconSave()` (`navigator.sendBeacon` final save, called from `main.js`'s `beforeunload`). Pure/Node-exported for Jest: `chatTitleFrom(history)`, `conversationRecordFrom(id, history, personaKey)` (mirrors `saveChat()`'s image-stripping mapper — `imageBase64`/`imageDataUrl` dropped, `hasImage` kept), `filterConversations(list, query, searchBody)`. Chat never blocks on this service; auto-save failures are silent and retried next turn. See [Conversation History](#conversation-history) |
| `main.js` | `DOMContentLoaded` wiring — all event listeners via `addEventListener`; **the handler is `async`: it calls `initSettings()`, `GET`s `SETTINGS_API_URL` into `window.vpalSettings` (falls back to `_defaultVpalSettings()` from `config.js` constants + a toast on failure), runs `_migrateLegacySettingsIfNeeded()` (one-time `localStorage` → server migration, guarded by `localStorage['settingsMigrated']`), then `initNavRail()` + `initHistory()` (history.js) + `applyResolvedSettings()` + `updateSystemPromptState()`. `#settingsMenuItem` / `#personaToggleBtn` / `#modelBadge` / `#thinkingBadge` all open `openSettings(...)`; `#historyMenuItem` → `closeProfileDropdown()` + `openHistory()`. A `beforeunload` listener calls `hcBeaconSave()` for a best-effort final save. The old per-control wiring (model selector, autoTTS, thinking toggle+depth, TTS engine, nav-rail toggle, editor-mode selector, persona `<select>` + `personaPrefs` snapshot/restore, persona panel open/close) and its now-dead helpers (`_setThinkingUI`, `_setTTSUI`, `_snapshotPersonaSettings`, `_applyPersonaSettings`, `_persistPersonaPref`, `_updateEditorModeVisibility`) are **gone** — those preferences now live in the Settings lightbox (`settings.js`) and persist server-side. `_trapFocus(panel, e)` remains for the profile dropdown and attach menu; `clearImagePreview()`/`clearDocumentPreview()` defined at top level (called cross-module by `chat.js`), both delegating to `_updateAttachMenuActiveState()` to toggle `#attachMenuBtn`'s active indicator; a single ChatGPT-style "+" trigger (`#attachMenuBtn`/`#attachMenuDropdown`, opened upward via `openAttachMenu()`/`closeAttachMenu()` — mirrors `openProfileDropdown()`/`closeProfileDropdown()`) replaces the old separate image/document toolbar buttons, offering "Add photos" and "Add files" menu items that each trigger their respective hidden `<input type="file">` then close the menu; the `documentInput` change handler validates extension/size, extracts text (`.txt`/`.md`/`.markdown` via `file.text()`; `.pdf` via a `POST` to `DOC_EXTRACT_URL`), applies `truncateDocumentText()`, and populates the preview strip |
...
Purely client-side (`src/aia/scripts/nav-rail.js`; see the [JavaScript Module Architecture](#javascript-module-architecture) row). A thin vertical strip of markers on the right inner edge of `.chat-messages` — one per **completed user turn** — for jumping around a long conversation.
...
A single **Settings lightbox** (opened from the profile dropdown, next to Sign out) is the one place every preference is edited. It replaced the scattered toolbar / persona-panel controls — `#modelSelect`, `#ttsEngineSelect`, `#autoTTSBtn`, `#thinkingModeBtn`, `#thinkingDepthSelect`, `#navRailToggle`, `#editorModeSelect`, the persona `<select>` and `#personaPanel` are all removed. The toolbar keeps only *actions* (mic, "+", stop-speaking, send, stop) plus two non-interactive **badges** (`#modelBadge`, `#thinkingBadge`, SVG-icon + text, click → open Settings).
...
An auto-saved, searchable list of past conversations, persisted **per authenticated user** by a new `conversations-service` (FastAPI + stdlib SQLite/WAL on the `vpal-conversations-data` named volume), reached through nginx at `https://localhost/conversations`. It is a convenience layer **on top of** the unchanged manual JSON Save / Export MD / Open in the profile menu.

- **Frontend** — `src/aia/scripts/history.js` (loaded after `settings.js`, before `main.js`), `CONVERSATIONS_API_URL` in `config.js`. See the `history.js` row in [JavaScript Module Architecture](#javascript-module-architecture). The lightbox mounts in `#historyRoot` (a child of `.chat-container`); `#historyMenuItem` in the profile dropdown opens it.
- **Auto-save on every turn** — `api.js` fires `hcTouchCurrent()` (fire-and-forget) at each of its three turn-complete points. The client mints the conversation id (`'c-' + timestamp + '-' + random`, validated `^[A-Za-z0-9_-]{1,64}$` server-side) so it can `PUT` from the first turn. `main.js`'s `beforeunload` calls `hcBeaconSave()` (`navigator.sendBeacon`) for a best-effort final save.
- **"New chat"** — the profile menu's old "Clear" item is now **"New chat"** (`#clearBtn` keeps its id; only text + icon changed). `clearChat()` is `async` and `await`s `hcArchiveCurrent()` — a `PUT` of the still-populated `conversationHistory` — *before* wiping, so nothing is lost and there is no destructive `confirm()` any more. Same for the `Ctrl/Cmd+Shift+O` shortcut.

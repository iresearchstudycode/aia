...
- `currentModel` — runtime global; the model sent on every text/thinking turn. Hydrated from the settings service; `MODEL_NAME` is the fallback. **Known limitation:** `options.num_ctx` is still sent as `OLLAMA_NUM_CTX` (16384) regardless of the chosen model's context window.

...
# 9 deployment placeholders. All other $-variables in the template are nginx
...
export AUTH_UPSTREAM VOICEBOX_PROXY_UPSTREAM DOC_EXTRACT_UPSTREAM PIPER_UPSTREAM SETTINGS_UPSTREAM OLLAMA_UPSTREAM SERVER_NAME SSL_CERT_PATH SSL_KEY_PATH

TEMPLATE="${1:-"$(dirname "$0")/nginx.conf.template"}"
exec envsubst '${AUTH_UPSTREAM} ${VOICEBOX_PROXY_UPSTREAM} ${DOC_EXTRACT_UPSTREAM} ${PIPER_UPSTREAM} ${SETTINGS_UPSTREAM} ${OLLAMA_UPSTREAM} ${SERVER_NAME} ${SSL_CERT_PATH} ${SSL_KEY_PATH}' < "$TEMPLATE"

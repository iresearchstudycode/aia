...
The auth, voicebox-proxy, doc-extract, piper-tts, and settings-service services each have a pytest suite. Run them from their own directory — `conftest.py` adjusts `sys.path` (and, for `auth/`, sets required env vars; for `settings-service/`, points `SETTINGS_DB_PATH` at a per-test tmp file) automatically.
...
CI runs seven jobs automatically on every push and PR to `master` (`.github/workflows/ci.yml`):
...
**Updating Chainguard base image digests** — the `FROM` lines in `auth/Dockerfile`, `voicebox-proxy/Dockerfile`, `doc-extract/Dockerfile`, and `settings-service/Dockerfile` are pinned to SHA256 digests (all currently pinned to the same Chainguard Python image). To update them after Chainguard releases a new image:
...
# Then rebuild: docker-compose build auth voicebox-proxy doc-extract settings-service
```

...
The docker-compose bind mount serves the working-copy bytes straight to the browser, so the served file must hash to exactly the `integrity=` value — a line-ending conversion between commit and checkout is enough to break it, and this repo's dev machine runs `core.autocrlf=true`. Current state, asset by asset: `marked.min.js` / `dompurify.min.js` are multi-line and their committed `integrity=` values match a **CRLF** working copy; `katex.min.js` / `katex-auto-render.min.js` have no newlines so CRLF conversion is a no-op and they match; `katex.min.css` has a single trailing newline and its `integrity=` is the **LF** hash, so an autocrlf checkout currently mismatches it (pre-existing, low-impact — the KaTeX CSS just fails to apply; tracked separately). `diff-match-patch.js` is marked `binary` in `.gitattributes` so git never touches its endings — its hash is the committed (LF) bytes and stays valid on every platform. If you see a browser SRI error, compare `git show HEAD:<path> | sha256sum`, the working-copy `sha256sum`, and the `integrity=` value — all three must agree. (Normalising every vendored asset to `binary` + LF-recomputed hashes, with a CI guard, is the proper fix and is tracked separately.)

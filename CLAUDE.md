...
| `config.js` | `MODEL_NAME`, `VISION_MODEL_NAME`, `OLLAMA_API_URL`, `OLLAMA_TAGS_URL`, `OLLAMA_MODEL_KEY`, `PIPER_SPEAK_URL`, `VOICEBOX_SPEAK_URL`, `DOC_EXTRACT_URL`, `PERSONA_PREFS_KEY`, `EDITOR_MODE_KEY`, the system prompt objects (11 dropdown personas + the non-dropdown `englishEditorExplained` variant), and global mutable state (`conversationHistory[]`, `currentSystemPrompt`, `currentModel` (defaults to `MODEL_NAME`; the actual model sent for text/thinking turns, set by the toolbar model selector), `pendingImageDataUrl`, `pendingImageBase64`, `currentThinkingMode`, `currentTTSEngine`, `currentEditorMode`, `pendingDocumentText`, `pendingDocumentName`, `pendingDocumentTruncated`) |

...
**Design tokens** — `style.css` now opens with a `:root` block (`--color-*`, `--space-*` (4px base), `--radius-*`, `--shadow-*`, `--focus-ring`, `--sidebar-width`, `--z-*`) that is the single source of truth for the previously-scattered literals. New surfaces + the Settings lightbox consume them; older component rules are migrated opportunistically.
...
**Config** — env vars, all optional (fallbacks baked in; documented in `.env.example`): `SETTINGS_DB_PATH` (default `/data/settings.db`) and the 8 house-default keys `VPAL_DEFAULT_CHAT_MODEL` (`gemma4:e4b`), `VPAL_DEFAULT_VISION_MODEL` (`gemma3:4b`), `VPAL_DEFAULT_TTS_ENGINE` (`piper`), `VPAL_DEFAULT_AUTO_SPEAK` (`false`), `VPAL_DEFAULT_STT_LANG` (`en-US`), `VPAL_DEFAULT_THINKING` (`false`), `VPAL_DEFAULT_THINKING_DEPTH` (`medium`), `VPAL_DEFAULT_NAV_RAIL` (`true`). `active_persona` always defaults to `englishEditor` (not env-configurable). Update the Chainguard digest pins the same way as the other services (see **Updating Chainguard base image digests**).

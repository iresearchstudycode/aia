...
| File | Purpose |
|---|---|
| `piper-tts/main.py` | FastAPI app. `POST /piper/speak` — JSON `{"text": "..."}` → raw `audio/wav` (`Response(media_type="audio/wav")`). `_sanitize()` strips ASCII control chars (keeps `\t`/`\n`/`\r`); the route rejects empty/whitespace text (400) and text over `PIPER_MAX_TEXT_CHARS` (413). `_get_voice()` lazily loads and caches the `PiperVoice` (kept out of import time so tests never need the model); `_synthesize()` writes a WAV into an in-memory buffer via `wave` + `PiperVoice.synthesize_wav(...)`, wrapping any failure as `SynthesisError` → 500. No storage, no cache, no second endpoint — v1 is deliberately minimal |
...
| `piper-tts/tests/test_main.py` | 13 pytest tests — `_sanitize`, `/health`, and every `/piper/speak` branch (valid → 200 + `audio/wav`, empty/whitespace → 400, over-length → 413, missing field → 422, synthesis failure → 500). **Synthesis is always mocked** (`_get_voice` / `_synthesize` via `monkeypatch`), so CI never downloads the ~60 MB model |

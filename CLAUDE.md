...
**These six assets are LF-pinned via `.gitattributes` (`text eol=lf`)** — the docker-compose bind mount serves the working-copy bytes directly, so they must match the committed blob on every platform (this repo's dev machine has `core.autocrlf=true`, which would otherwise give the multi-line ones CRLF working copies and a hash mismatch → the browser refuses to load the script). **The SRI hash must be computed from the LF bytes.** `[IO.File]::ReadAllBytes` on a CRLF working copy (e.g. right after an `autocrlf` checkout of an unpinned file) produces the wrong hash — the committed blob is the source of truth. To verify after any change: `git show HEAD:<path> | sha256sum` must equal both the working-copy `sha256sum` and the `integrity=` value in `index.html`.

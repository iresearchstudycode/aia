...
// NEW: Centralized function to update system prompt selector state
...
function addAIMessagePlaceholder() {
  ...
  messageDiv.innerHTML = `
      ...
      <div class="message-content">
        <p style="opacity: 0.5;">Thinking...</p>
      </div>
      <div class="message-timestamp">${timestamp}</div>
    `;
  messagesDiv.appendChild(messageDiv);
...
function addAIMessage(markdownText) {
  ...
  const timestampFmt = formatTimestamp(new Date());
  const messageDiv = document.createElement('div');
  messageDiv.className = 'message ai-message';
  messageDiv.innerHTML = `
            <div class="message-label">AI Assistant</div>
            <div class="message-content"></div>
            <div class="message-timestamp">${timestampFmt}</div>
        `;
  messagesDiv.appendChild(messageDiv);
  ...
  // Store assistant message in conversation history with timestamps
  conversationHistory.push({
    role: 'assistant',
    content: markdownText,
    timestamp: timestampISO,
    formattedTimestamp: timestampFmt
  });
